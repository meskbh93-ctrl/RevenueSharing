...
export function calculateServiceRevenue(service) {
  ...
  let quantity =
    service.quantity ??
    service.base_quantity ??
    0;

  const priceGrowth =
    service.price_growth_rates ||
    [0, 0, 0, 0, 0];

  const quantityGrowth =
    service.quantity_growth_rates ||
    [0, 0, 0, 0, 0];
...
export function calculateCostOverYears(
  cost
) {
  const years = [];
