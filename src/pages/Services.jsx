...
export default function Services() {
  ...
  const { data: services = [] } = useQuery({
    ...
    queryFn: () =>
      base44.entities.Service.filter({ project_id: selectedProjectId }),
    enabled: !!selectedProjectId,
  });
  ...
  const createMutation = useMutation({
    mutationFn: (data) =>
      base44.entities.Service.create({ ...data, project_id: selectedProjectId }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['services', selectedProjectId] });
      setFormOpen(false);
    },
  });
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-muted-foreground gap-3">
        ...
        <p className="text-lg font-medium">
          {lang === 'ar' ? 'لم يتم اختيار مشروع' : 'No project selected'}
        </p>
